...
const getPokeByApi = async (req, res) => {
    ...
    try {
        if (name !== undefined && name && name !== '') {
            ...
            if (pokemon)res.json(pokemon);
                ...
                let types = url.data.types.map(el => el.type.name)
    ...
    details = details.map(poke => {
        ...
        return newPokemon = { ...newPokemon, types: newTypes };
    })
...
const getTypes = async (req, res, next) => {
    ...
    const datos = await Type.bulkCreate(pokeTipos) //acá es donde me lo guardo en mi base de datos
